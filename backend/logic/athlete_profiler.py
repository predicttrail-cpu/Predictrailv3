...
async def get_athlete_profile_logic(token: str, manual_ids: List[int]) -> AthleteProfile:
    from .strava_utils import fetch_strava_api
    ...
    try:
        ...
        for act_id in activity_ids_to_fetch:
            try:
                streams.append(fetch_strava_api(token, f"activities/{act_id}/streams?keys=time,latlng,distance,altitude,heartrate,velocity_smooth,cadence&key_by_type=true"))
            except Exception as e:
                if hasattr(e, 'status_code') and e.status_code == 404:
                    print(f"Avertissement: Activité {act_id} ignorée (404).")
                else:
                    raise
        ...
        if hasattr(e, 'status_code'):
            raise e
        raise Exception(f"Erreur de profilage: {e}")
